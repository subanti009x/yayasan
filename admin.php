...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'login') {
        ...
        if ($uploadError !== null) {
            ...
            $error = 'Artikel belum bisa disimpan. Pastikan folder data dapat ditulis oleh server.';
        }
    }
}
...
if (($_GET['success'] ?? '') === 'save') {
    $message = 'Artikel berhasil disimpan dan akan otomatis tampil jika statusnya Published.';
                ...
                <div class="grid gap-8 lg:grid-cols-[0.95fr_1.05fr]">
                    <form method="post" enctype="multipart/form-data" class="rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
                        ...
                        <div class="mt-6 flex flex-col gap-3 sm:flex-row">
                            ...
                            <?php if ($editArticle): ?>
                                <a href="admin.php" class="rounded-md border border-slate-200 px-5 py-3 text-center text-sm font-bold text-slate-700 transition hover:border-amber-300 hover:text-orange-700">Batal Edit</a>
                            <?php endif; ?>
                        </div>
                    </form>
                                ...
                                <article class="rounded-lg border border-slate-200 bg-slate-50 p-4">
                                    <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                                        ...
                                        <div class="flex shrink-0 gap-2">
                                            <a href="admin.php?edit=<?= e($article['id'] ?? ''); ?>" class="rounded-md border border-slate-200 bg-white px-3 py-2 text-xs font-bold text-slate-700 transition hover:border-amber-300 hover:text-orange-700">Edit</a>
